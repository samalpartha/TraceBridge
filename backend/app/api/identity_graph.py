...
import hashlib, math, random, logging
...
async def get_identity_graph(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await db.execute(
            select(Case).options(selectinload(Case.media_assets))
        )
        ...
        for sr in sources:
            ...
            if sid not in node_ids:
                nodes.append(_node(sid, sr.person_name or f"Record {str(sr.id)[:8]}", "sighting",
            ...
            if sr.location_name:
                ...
                if slid not in node_ids:
                    nodes.append(_node(slid, sr.location_name, "location",
                                       lat=sr.location_lat, lng=sr.location_lng))
                    node_ids.add(slid)
        ...
        logger.error(f"Error building identity graph: {str(e)}", exc_info=True)
        raise
